...
from .serializers import ChangePasswordSerializer, PasswordCheckEmailSerializer, PasswordCodeCheckSerializer
...
class PasswordResetEmailView(APIView):
    ...
    @swagger_auto_schema(responses={"201": "created", "400": "bad request", "404": "not found"}
        , operation_description="for create code and send mail", request_body=PasswordCheckEmailSerializer)
        ...
        try:
            ...
            try:
                ...
                queries = OTP.objects.filter(
                    created_time__lte=datetime.datetime.fromtimestamp(timezone.now().timestamp() - 60, tz=timezone.utc))
                ...
                return Response(data="Code expired after 60 seconds", status=status.HTTP_201_CREATED)
            except ObjectDoesNotExist:
                return Response(data=PasswordResetEmailView.on_error, status=status.HTTP_404_NOT_FOUND)
        except KeyError:
            return Response(data=PasswordResetEmailView.on_error, status=status.HTTP_400_BAD_REQUEST)
...
class PasswordResetCheckCodeView(APIView):
    ...
    # for check otp
    @swagger_auto_schema(responses={"200": "Ok", "400": "bad request", "404": "not found"},
                         operation_description="for check code", security=[], request_body=PasswordCodeCheckSerializer)
    def post(self, request):
        try:
...
class PasswordChangeView(APIView):
    ...
    @swagger_auto_schema(responses={"200": "Ok", "400": "bad request", "404": "not found"},
                         operation_description="for change password", request_body=ChangePasswordSerializer)
    def post(self, request):
        try:
            req_email = request.session["email"]
